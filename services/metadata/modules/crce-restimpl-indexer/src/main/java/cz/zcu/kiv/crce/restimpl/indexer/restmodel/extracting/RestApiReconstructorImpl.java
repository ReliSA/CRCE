...
public class RestApiReconstructorImpl implements RestApiReconstructor {
     ...
     */
    public RestApiReconstructorImpl(Map<String, ClassStruct> classesMap,
            WebXmlParser.Result webXmlResult) {
    ...
    private Map<String, RestApiDefinition> loadDefinitions() {
        ...
        URL[] urls =
                new URL[] {getClass().getClassLoader().getResource(DEFS_DIR + "/jaxrs_api.yml"),
                        getClass().getClassLoader().getResource(DEFS_DIR + "/spring_api.yml")};

        for (URL fileUrl : urls) {
            try {
                RestApiDefinition restApiDefinition =
                        mapper.readValue(fileUrl, RestApiDefinition.class);
    ...
    public Collection<Endpoint> extractEndpoints() {
        ...
        this.interpreter =
                new MethodBodyInterpreter(classModelProcessor.getDefinition(), classesMap); // the correct definition has been picked by now
    ...
    private void detectResources(Collection<ClassStruct> classes) {
        ...
        if (!resources.isEmpty()) {
            exceptionsMappings =
                    classModelProcessor.prepareExceptionsMappings(exceptionHandlers, classesMap);
        }
    }
     ...
     */
    private void processEndpoint(PathPartAttributes resourceAttributes, Method method,
            Collection<Endpoint> endpoints) {
        Endpoint endpoint = new Endpoint();

        endpoint.setName(method.getDescription());
        ...
        endpoint.setHttpMethods(getEndpointMethods(resourceAttributes.getMethods(),
                endpointAttributes.getMethods()));
        // paths are appended
        endpoint.setPaths(
                getEndpointPaths(resourceAttributes.getPaths(), endpointAttributes.getPaths()));
        // MIME types are overwritten
        endpoint.setConsumes(
                getMimeTypes(resourceAttributes.getConsumes(), endpointAttributes.getConsumes()));
        endpoint.setProduces(
                getMimeTypes(resourceAttributes.getProduces(), endpointAttributes.getProduces()));
        ...
        Set<EndpointResponse> responses = classModelProcessor
                .getResponsesFromExceptions(method.getExceptions(), exceptionsMappings);
     ...
     */
    private boolean processReturnType(Method method, Endpoint endpoint,
            Collection<Endpoint> endpoints) {
        ...
        if (clazz != null && classModelProcessor.isSubresource(clazz)) {
            ...
            resourceAttributes
                    .setPaths(getEndpointPaths(endpoint.getPaths(), resourceAttributes.getPaths()));
     ...
     */
    private Set<String> getEndpointMethods(List<String> resourceMethods,
        ...
        for (Variable variable : method.getParameters()) {
            if (classModelProcessor.isRequestParameter(variable)) {
                RequestParameter parameter =
                        classModelProcessor.convertParameter(variable, endpoint.getConsumes());
                ...
                if (bodyDataType.isStructured()) {
                    if (BytecodeDescriptorsProcessor
                            .isArrayOrCollection(bodyDataType.getBasicType())) {
                        body.setArray(true);
                ...
                body.setOptional(!variable.getAnnotations()
                        .containsKey(classModelProcessor.getDefinition().getNotNullAnnotation()));
                ...
                Set<RequestParameter> beanParameters = classModelProcessor
                        .getParamsFromBean(endpoint, variable, pathVariables, classesMap);
    ...
    private void setDefaultMimeTypes(Endpoint endpoint, RestApiDefinition definition) {
        ...
        if (endpoint.getConsumes().isEmpty() && requestBody != null) {
            setDefaultMimeTypes(requestBody.getStructure(), requestBody.isArray(),
                    endpoint.getConsumes(), definition);
        }
        if (endpoint.getProduces().isEmpty()) {
            ...
            for (EndpointResponse response : responses) {
                setDefaultMimeTypes(response.getStructure(), response.isArray(),
                        endpoint.getProduces(), definition);
            }
        }
    }

    private void setDefaultMimeTypes(String dataType, boolean isArray, Set<String> mimeTypes,
            RestApiDefinition definition) {
    ...
    private void addPrefixesToEndpoints(Collection<Endpoint> endpoints) {
        ...
        for (Endpoint endpoint : endpoints) {
            List<String> paths =
                    getEndpointPaths(webXmlResult.getUrlPrefixes(), endpoint.getPaths());
