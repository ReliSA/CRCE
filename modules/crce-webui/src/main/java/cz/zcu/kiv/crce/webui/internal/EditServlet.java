...
//			Zjištění zda kategorie byla odstraněna.
    ...
    private boolean deleteComponent(HttpServletRequest req, HttpServletResponse resp, final Map<?, ?> parameters) {
        ...
        try {
            ...
            } else if ("webservices".equals(link)) {
                List<Resource> array = Activator.instance().getWsBuffer(req).getResources();
                Resource found = findResource(fileUri, array);
                if (!Activator.instance().getWsBuffer(req).remove(found)) {
                    logger.warn("Webservice buffer didn't contain removed resource: {}", found);
                }
            } else {
                return false;
            }
    ...
    private List<Resource> getResources(HttpServletRequest req, String link) {
        switch (link) {
            ...
            case "webservices":
                return Activator.instance().getWsBuffer(req).getResources();
            default:
                return null;
        }
    }
