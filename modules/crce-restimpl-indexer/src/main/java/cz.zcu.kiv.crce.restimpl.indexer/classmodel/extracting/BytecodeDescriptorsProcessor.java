...
public class BytecodeDescriptorsProcessor {
    ...
    /**
     *
     * Method processes method descriptor and sets parameter data types and return type to the given method.
    ...
    public static void processMethodDescriptor(String desc, Method method) {
        if (desc.contains("T;") || desc.contains(":")) return;
        try {
    ...
    /**
     * Processes method descriptor (or method signature) and retrieves information
     * about method return type and parameter types.
    ...
    private static String getInnerGenericType(String classTypeSign) {
        String inner = classTypeSign.substring(classTypeSign.indexOf("<") + 1, classTypeSign.lastIndexOf(">" ));
     ...
     * @return data type wrapper containing the last position of field descriptor in parameters descriptor
     */
        ...
        try {
            ...
            if (matcher.find() && matcher.start() == 0) {
                ...
                if (!part.contains("<")) {
                    ...
                    DataType innerType = new DataType(BytecodeDescriptorsProcessor.getInnerGenericType(part));
    ...
    public static boolean isArrayOrCollection(String type) {
        ...
        if ("java.util.Collection".equals(type)) return true;
        try {
