...
public class RestimplMetadataManager {
    ...
    public void setMetadata(Resource resource, Collection<Endpoint> endpoints, String framework) {
        ...
        Capability restimplIdentityCapability =
                metadataFactory.createCapability(RestimplMetadataConstants.NS__RESTIMPL_IDENTITY);
        restimplIdentityCapability.setAttribute(RestimplMetadataConstants.ATTR__RESTIMPL_FRAMEWORK,
                framework);
    ...
    private Capability createEndpointCapability(Endpoint endpoint) {
        Capability endpointCapability =
                metadataFactory.createCapability(RestimplMetadataConstants.NS__RESTIMPL_ENDPOINT);

        // set attributes (name, paths, methods, consumes, produces)
        setIfSet(endpointCapability, RestimplMetadataConstants.ATTR__RESTIMPL_NAME,
                endpoint.getName());
        setIfSet(endpointCapability, RestimplMetadataConstants.ATTR__RESTIMPL_ENDPOINT_PATH,
                endpoint.getPaths());
        setIfSet(endpointCapability, RestimplMetadataConstants.ATTR__RESTIMPL_ENDPOINT_METHOD,
                new ArrayList<>(endpoint.getHttpMethods()));
        setIfSet(endpointCapability, RestimplMetadataConstants.ATTR__RESTIMPL_ENDPOINT_CONSUMES,
                new ArrayList<>(endpoint.getConsumes()));
        setIfSet(endpointCapability, RestimplMetadataConstants.ATTR__RESTIMPL_ENDPOINT_PRODUCES,
                new ArrayList<>(endpoint.getProduces()));
        setIfSet(endpointCapability,
                RestimplMetadataConstants.ATTR__RESTIMPL_ENDPOINT_ENDPOINT_DEPENDENT,
        ...
        for (EndpointResponse response : endpoint.getResponses()) {
            ...
            for (EndpointParameter parameter : response.getParameters()) {
                addPropertyIfSet(endpointCapability,
                        createResponseParamProperty(parameter, responseId));
            }
        }
    ...
    private Property createBodyProperty(EndpointRequestBody body) {
        ...
        Property bodyProperty =
                metadataFactory.createProperty(RestimplMetadataConstants.NS_RESTIMPL_REQUEST_BODY);
        setIfSet(bodyProperty, RestimplMetadataConstants.ATTR__RESTIMPL_DATETYPE,
                body.getStructure());
        setIfSet(bodyProperty, RestimplMetadataConstants.ATTR__RESTIMPL_ARRAY,
                body.isArray() ? (long) 1 : 0);
        setIfSet(bodyProperty, RestimplMetadataConstants.ATTR__RESTIMPL_OPTIONAL,
                body.isOptional() ? (long) 1 : 0);
        return bodyProperty;
    }

    private Property createRequestParamProperty(RequestParameter parameter) {
        Property property = createParamProperty(parameter,
                RestimplMetadataConstants.NS_RESTIMPL_REQUESTPARAMETER);
        if (parameter == null) {
            return null;
        }
        setIfSet(property, RestimplMetadataConstants.ATTR__RESTIMPL_DEFAULT_VALUE,
                parameter.getDefaultValue());
        setIfSet(property, RestimplMetadataConstants.ATTR__RESTIMPL_OPTIONAL,
                parameter.isOptional() ? (long) 1 : 0);
        return property;
    }

    private Property createResponseParamProperty(EndpointParameter parameter, String responseId) {
        Property property = createParamProperty(parameter,
                RestimplMetadataConstants.NS_RESTIMPL_RESPONSEPARAMETER);
        if (parameter == null) {
            return null;
        }
    ...
    private Property createParamProperty(EndpointParameter parameter, String propertyNamespace) {
        ...
        setIfSet(paramProperty, RestimplMetadataConstants.ATTR__RESTIMPL_DATETYPE,
                parameter.getDataType());
        setIfSet(paramProperty, RestimplMetadataConstants.ATTR__RESTIMPL_PARAMETER_CATEGEORY,
                parameter.getCategory().toString());
        setIfSet(paramProperty, RestimplMetadataConstants.ATTR__RESTIMPL_ARRAY,
                parameter.isArray() ? (long) 1 : 0);
    ...
    private Property createResponseProperty(EndpointResponse response, String id) {
        ...
        Property responseProperty =
                metadataFactory.createProperty(RestimplMetadataConstants.NS_RESTIMPL_RESPONSE);
        setIfSet(responseProperty, RestimplMetadataConstants.ATTR__RESTIMPL_RESPONSE_ID, id);
        setIfSet(responseProperty, RestimplMetadataConstants.ATTR__RESTIMPL_DATETYPE,
                response.getStructure());
        setIfSet(responseProperty, RestimplMetadataConstants.ATTR__RESTIMPL_RESPONSE_STATUS,
                (long) response.getStatus());
        setIfSet(responseProperty, RestimplMetadataConstants.ATTR__RESTIMPL_ARRAY,
                response.isArray() ? (long) 1 : 0);
    ...
    protected <T> boolean setIfSet(Capability capability, ListAttributeType attribute,
            List<String> values) {
