...
public class MavenStoreImpl implements Store {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(MavenStoreImpl.class);
	...
	MavenStoreImpl(URI baseUri) throws IOException {
		...
		if (!mvnStorePath.exists()) {
			if (!mvnStorePath.mkdirs()) {
				logger.error("Could not create maven store directory {}",
						mvnStorePath);
			}
		...
		if (!mvnStorePath.exists()) {
			throw new IllegalStateException(
					"Base direcotory could be not created: " + mvnStorePath,
					new IOException("Can not create directory"));
		}
	}

	@Override
	public Resource put(Resource resource) throws IOException,
			RefusedArtifactException {
	...
	public List<Resource> getResources() {
		try {
			...
			logger.error("Could not load resources of repository {}.", baseUri,
					e);
		}
	...
	public List<Resource> getResources(Requirement requirement) {
		...
		try {
			...
			logger.error("Could not load resources for requirement ({})",
					requirement.getNamespace(), e);
		}

		if (logger.isDebugEnabled()) {
			logger.debug("getResources(requirement={}) returns {}",
					requirement.getNamespace(), resources.size());
		}
		return resources;
	}

	@Override
	public void execute(List<Resource> resources, Executable executable,
	...
	synchronized void start() {
		...
		repositoryIndexer = createRepositoryIndexer(baseUri);

		for (Artifact a : repositoryIndexer.getArtifacts()) {
			index(a.getFile());
		}
	...
	private void index(File file) {
		try {
			if (resourceIndexerService != null
					&& !resourceDAO.existsResource(file.toURI())) {
				Resource resource;
				try {
				...
				metadataService.getIdentity(resource).setAttribute(
						"repository-id", String.class, repository.getId());

				identityIndexer.preIndex(file, file.getName(), resource);
				identityIndexer.postIndex(file, resource);

				ResourceValidationResult validationResult = metadataValidator
						.validate(resource);
				...
				try {
					...
					logger.error(
							"Could not save indexed resource for file {}: {}",
							file, resource, e);
				}
			}
		} catch (IOException e) {
	...
	public LocalMavenRepositoryIndexer createRepositoryIndexer(
			URI repositoryBaseDir) {
		LocalMavenRepositoryIndexer repositoryIndexer = new LocalMavenRepositoryIndexer(
				repositoryBaseDir);
